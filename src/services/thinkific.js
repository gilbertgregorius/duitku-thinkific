...
class Thinkific {
  ...
  async getUserByEmail(accessToken, email) {
    try {
      ...
      logger.error('[service/thinkific] Error fetching user by email:', error.message);
      throw error;
    }
  }
  ...
  async createExternalOrder({ customerEmail, productId, amount, reference }) {
    try {
      const accessToken = await tokenManager.getToken();
      if (!accessToken) {
        logger.error('[service/thinkific] Access token is not available');
        throw new Error('Access token is not available');
      }
      const user = await this.getUserByEmail(accessToken, customerEmail);
      const headers = this.createHeaders(accessToken);
      const response = await axios.post(
      ...
      return response.data; // contain {id}
    } catch (error) {
  ...
  async isCrouse(accessToken, productId) {
    try {
  ...
  async enrollUserInProduct(userId, productId, accessToken) {
    try {
      const courseId = await this.isCrouse(accessToken, productId);
